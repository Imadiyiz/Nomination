...
class TestBiddingManager():
    ...
    def test_successful_player_bid(self, bm, human_player):
        ...
        #no handicap
        assert bm.successful_player_bid(human_player, not_allowed=2, bid_amount=2) # not handicapped
        ...
        assert not bm.successful_player_bid(human_player, not_allowed=2, bid_amount=2)  # handicapped
        assert human_player.bid != 2

        #Invalid bid: bid is too high
        assert not bm.successful_player_bid(human_player, not_allowed=3, bid_amount=10) 
        ...
        assert bm.successful_player_bid(human_player, not_allowed=3, bid_amount=2) 
        assert human_player.bid == 2
